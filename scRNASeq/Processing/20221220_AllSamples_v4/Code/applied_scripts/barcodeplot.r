## function copied from version 3.20.1 of limma, changes indicated with PM
barcodeplot = function (statistics, index, index2 = NULL, labels = c("Up", 
                                                       "Down"), quantiles = c(-1, 1), col.bars = NULL, worm = TRUE, 
          span.worm = 0.45, ...) 
{
  TWO <- !is.null(index2)
  ...
  if (TWO) 
    ylim[1] <- ylim[1] - 0.5
  plot(1:n, xlim = c(0, n), ylim = ylim.worm, type = "n", axes = FALSE, 
  ...
  if (!TWO) 
    rect.yb <- 0
  rect(npos + 0.5, rect.yb, n - nneg + 0.5, 0.5, col = "lightgray", 
       border = NA)
  if (npos) 
    ## PM: switched lightblue and pink
    rect(0.5, rect.yb, npos + 0.5, 0.5, col = "lightblue", border = NA)
  if (nneg) 
    rect(n - nneg + 0.5, rect.yb, n + 0.5, 0.5, col = "pink", 
         border = NA)
  lwd <- 50/length(r)
  lwd <- min(2, lwd)
  lwd <- max(0.1, lwd)
  ...
  if (TWO) {
    ...
    segments(r2, barlim2[1], r2, barlim2[2], lwd = lwd2, 
             col = col.bars[2])
  }
  lab.at <- 0
  if (!TWO) 
    lab.at <- 0.5
  axis(side = 2, at = lab.at, padj = 3, cex.axis = 0.85, labels = labels[1], 
       tick = FALSE)
  axis(side = 4, at = lab.at, padj = -3, cex.axis = 0.85, labels = labels[2], 
       tick = FALSE)
  ## PM: replaced 10:0 by 0:10
  prob <- (0:10)/10
  ## PM: flipped sign back
  ...
  if (worm) {
    ...
    worm1.scale <- rescale(worm1, newrange = c(1.1, 2.1), 
                           oldrange = r.worm1)
    if (TWO) {
      min.worm2 <- min(worm2)
      r.worm2 <- c(min.worm2, 0)
      worm2.scale <- rescale(worm2, newrange = c(-2.1, 
                                                 -1.1), oldrange = r.worm2)
    }
    if (!TWO) {
      lines(x = 1:n, y = worm1.scale, col = col.bars[1], 
            lwd = 2)
      abline(h = rescale(1, newrange = c(1.1, 2.1), oldrange = r.worm1), 
             lty = 2)
      axis(side = 2, at = c(1.1, 2.1), cex.axis = 0.8, 
           labels = c(0, format(max.worm1, digits = 2)))
      axis(side = 2, labels = "Enrichment", at = 1.6, padj = -0.6, 
    ...
    if (TWO) {
      ...
      lines(x = 1:n, y = worm2.scale, col = col.bars[2], 
            lwd = 2)
      abline(h = rescale(-1, newrange = c(-2.1, -1.1), 
                         oldrange = r.worm2), lty = 2)
      axis(side = 2, at = c(1.1, 2.1), cex.axis = 0.7, 
           labels = c(0, format(max.worm1, digits = 2)))
      axis(side = 2, at = c(-1.1, -2.1), cex.axis = 0.7, 
           labels = c(0, format(-min.worm2, digits = 2)))
      axis(side = 2, labels = "Enrichment", at = -1.6, 
           tick = FALSE, padj = -0.6, cex.axis = 0.7)
      axis(side = 2, labels = "Enrichment", at = 1.6, tick = FALSE, 
           padj = -0.6, cex.axis = 0.7)
    }
  }
  invisible()
}
